...
def reorderLogFiles(self, logs):
  ...
  all lower case letters: 26 characters -> maybe O(1) space
  digits: number? positive or negative
  """
  ...
  letter_logs = self.customized_bubble_sort(letter_logs)
  return letter_logs + digit_logs
  

def customized_bubble_sort(self, letter_logs):
  for i in range(len(letter_logs)):
    j = i
    while j > 0 and self.log_1_is_smaller(letter_logs[j], letter_logs[j - 1]):
      self.swap(j, j - 1, letter_logs)
      j -= 1
  return letter_logs

def swap(self, i, j, logs):
  temp = logs[i]
  logs[i] = logs[j]
  logs[j] = temp
  
def log_1_is_smaller(self, i, j):
  log_1 = i.split(" ", 1)
  log_2 = j.split(" ", 1)
  if log_1[1] == log_2[1]:
    return log_1[0] < log_2[0]
  return log_1[1] < log_2[1]
